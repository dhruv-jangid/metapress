...
export class CacheService {
  ...
  static mGet = async <T>(keys: Array<string>): Promise<Array<T | null>> => {
  ...
  static zAdd = async (key: string, members: Array<{ score: number; value: string }>) => {
  ...
  static hSet = async (key: string, data: Array<string>) => {
    await redis.hSet(key, data);
  };

  static lRange = async <T>(key: string, start: number, end: number): Promise<Array<T>> => {
  ...
  static lPushMultiple = async <T>(key: string, items: Array<T>): Promise<number> => {
