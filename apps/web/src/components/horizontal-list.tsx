...
export const HorizontalList = ({
  ...
  data: Array<{ image: string; tag?: string; title: string; content?: string }>;
  imageOnly?: boolean;
}) => {
  return (
    <Carousel
        ...
        {data.map(({ image, title, tag, content }, i) => (
          ...
          >
            <Image
              ...
              className="object-cover rounded-xl aspect-2/3 border w-40 md:w-52 sepia-50 brightness-90"
            />
            <div
              className={cn(
            ...
            >
              <div className="space-y-1">
                <div className="text-xs md:text-sm tracking-tight font-medium">{tag}</div>
                <div className="text-2xl md:text-3xl xl:text-4xl font-medium tracking-tighter">
                  {title}
                </div>
                <div className="mt-4 text-sm text-balance max-w-2xs leading-tight">{content}</div>
              </div>
              <div className="text-8xl xl:text-9xl -mb-3.5 md:-mb-4">{i + 1}</div>
            </div>
          </CarouselItem>
        ))}
      </CarouselContent>
