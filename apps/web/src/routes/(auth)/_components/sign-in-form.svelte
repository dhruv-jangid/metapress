...
<FieldGroup class="bg-card ring-foreground/10 w-sm rounded-4xl p-8 sm:w-md md:w-lg">
  <form onsubmit={handleFormSubmit}>
    <FieldSet>
      ...
      <FieldDescription class="text-base tracking-tight">Express your creativity</FieldDescription>
      ...
      <FieldGroup>
        ...
        <Field>
          <FieldLabel for="password" class="items-start justify-between pr-1">
            Password
            <a
              href="/forget-password"
              class="tracking-tight underline decoration-dashed underline-offset-2 opacity-80"
            >
              Forgot password?
            </a>
          </FieldLabel>
      ...
      <FieldGroup class="gap-3">
        <Field orientation="horizontal" class="ml-0.5 opacity-80">
          <Checkbox id="rememberme" name="rememberme" {disabled} checked />
          <label for="rememberme" class="mt-[0.1rem] text-sm leading-tight"> Remember me </label>
        </Field>
        <Field orientation="horizontal" class="justify-between">
          <Button type="submit" {disabled} class="px-4">
            {#if loading.form}
              <Spinner />
            {:else}
              <HugeiconsIcon icon={LogIn} class="-ml-0.5" /> Sign in
            {/if}
          </Button>
