...
export class CacheService {
  static pipeline: () => ReturnType<typeof redis.multi> = () => redis.multi();

  static eval: typeof redis.eval = redis.eval.bind(redis);
  ...
  static mGet = async <T>(keys: string[]): Promise<(T | null)[]> => {
    ...
    return results.map((r) => (r ? JSON.parse(r as string) : null));
  };
  ...
  static sAdd = async (key: string, ...members: string[]) => {
    await redis.sAdd(key, members);
  };

  static sRem = async (key: string, ...members: string[]) => {
