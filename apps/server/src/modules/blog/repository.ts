...
import { blogImages, blogs, likes } from "@metapress/db/schemas";
import { and, desc, eq, gte, inArray, lt, sql } from "drizzle-orm";

import type { CreateBlogData, DeleteBlogData, UpdateBlogData } from "./types";
...
export abstract class BlogRepository {
  ...
  static async findManyRandom(limit: number) {
    const rows = await db
      ...
      .from(blogs)
      .where(gte(blogs.createdAt, sql`NOW() - INTERVAL '5 YEARS'`))
      .orderBy(sql`RANDOM()`)
      .limit(limit);
  ...
  static async update(data: UpdateBlogData) {
    const row = await db.transaction(async (tx) => {
      const [blog] = await tx
        .update(blogs)
        .set({
  ...
  static async delete(data: DeleteBlogData) {
